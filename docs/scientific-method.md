...
Candidates include simpler peak counts and polynomial baseline orders 0–2 in automatic peak-count mode. Exact peak-count mode fits only the requested count. Fast, Standard, and Thorough modes screen bounded candidate pools before refining finalists; Exhaustive mode fits every configured count directly. Windowed progressive fitting can propose starts from local cores and halos, but local scores and baselines are not exported as global evidence. Every recommended model is jointly refined and scored against the complete original spectrum with one global baseline.
...
Version 0.3 is validated on deterministic, identifiable synthetic cases. It makes no claim of super-resolution, technique-specific physics, causal peak assignment, or reliable recovery when the data do not identify a unique decomposition.
