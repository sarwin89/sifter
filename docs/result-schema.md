...
`fit_spectrum(...)` returns `FitResult.schema_version == "sifter.fit_result.v3"` for
SIFTER 0.4.2. `autofit(...)` remains compatibility-tested and returns
`sifter.fit_result.v2`; explicit legacy `sifter.fit_result.v1` serialization remains
compatibility-tested and omits newer top-level measurement and reference blocks.

The result records complete analysis settings and seed, manual/clicked peak-center hints, privacy-safe input metadata, optional measurement context, optional fit-reference provenance, input axes, the recommended model, retained top candidate models for visualization, all candidate score rows, optional Fourier diagnostics, residual diagnostics, parameter uncertainty, structured warnings, observation count, and SIFTER version.
...
In the v0.4.2 workbench path, retained candidate models are quality-ordered for user review: severe fit notes, worst local area error, median height error, normalized RMSE, peak count, and then BIC. BIC/AICc remain serialized for reproducibility and advanced diagnostics. Fast mode may emit `FAST_APPROXIMATE_FIT` when it uses a bounded polishing budget for interactivity; use `standard`, `thorough`, or `research` mode when fully converged optimizer evidence is preferred over speed.
